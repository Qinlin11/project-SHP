                ...
                <div class="InfoWrap">
                    ...
                    <div class="choose">
                        ...
                        <div class="cartWrap">
                            ...
                            <div class="add">
                                <!--以前的路由跳转， 从A路由跳转到B路由，这里添加购物车，
                                进行路由跳转之前，发请求把购买的产品信息通过请求方式通知服务器，服务器进行响应的储存-->
...
export default {
    ...
    methods: {
        ...
        //修改产品的个数
        changeSkuNum(event) {
        //用户输入进来的文本 * 1，判断用户输入是否是非法的
            let value = event.target.value * 1
            //非法为出现 NaN或者小于 1， 或者为空
        ...
        //加入购物车的回调函数
         async addShopCar() {
             //发请求---将产品加入到数据库（通知服务器）
             /*
             当前这里派发一个action，也向服务器发请求，判断加入购车是成功还是失败，进行相应的操作
             this.$store.dispatch('addOrUpdateShopCart', {skuId: this.$route.params.skuid, skuNum: this.skuNum})
             上面这行代码说白了：调用了仓库中addOrUpdateShopCart，这个方法加上asyc，返回一定是一个promise
             要么成功| 要么失败
             */
             try {
                 ...
                 //跳转的时候还要将产品信息带给下一级的组件addcartsuccess
